...
public class Chat implements Listener {
    ...
    private void onChat(AsyncPlayerChatEvent e) {
        ...
        String prefix = ChatColor.translateAlternateColorCodes('&', Configuration.getPrefixForPlayerRank(player));
        int radio_frequency = -1;

        if(MyZ.instance.getSQLManager().inClan(player.getUniqueId())) {
            String clanName = MyZ.instance.getSQLManager().getClan(player.getUniqueId());
            prefix = clanName + " " + prefix;
        }
        ...
        if ((Boolean) Configuration.getConfig("chat.format"))
            e.setFormat(prefix + e.getPlayer().getName() + ChatColor.WHITE + ": " + e.getMessage());
    ...
    private boolean didHandlePrivateChat(AsyncPlayerChatEvent e) {
        ...
        if (e.getMessage().startsWith("@")) {
            ...
            return true;
        } else if (e.getMessage().startsWith(".") && MyZ.instance.getSQLManager().inClan(player.getUniqueId())) {
            PlayerData data = PlayerData.getDataFor(player);
            if (data != null) {
                if ((Boolean) Configuration.getConfig("chat.format"))
                    e.setFormat(ChatColor.translateAlternateColorCodes('&', MyZ.instance.getConfig().getString("localizable.private.clan_prefix") + " "
            ...
            if (MyZ.instance.getSQLManager().isConnected()) {
                if ((Boolean) Configuration.getConfig("chat.format")) {
                    ...
                    e.setFormat(ChatColor.translateAlternateColorCodes('&', clanName + " "
                            + Configuration.getPrefixForPlayerRank(player) + ": " + e.getMessage().replaceFirst(".", "")));
                }
                e.getRecipients().clear();
